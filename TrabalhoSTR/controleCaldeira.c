...
void thread_controle_temperatura (void){
	...
	while(1){
		...
		//no = sensor_get_No();
		erro = (temp-ref_temp)/ref_temp;
		if(erro<0.0000) erro = -erro;
		//erro = abs((temp-ref_temp));
		double Ti = sensor_get_Ti();

      if(erro>0.2 && temp>ref_temp) { //diminui temperatura
    		atuador_put_Q(0.0);
    		if(Ti>temp && temp>80.0){
    			...
    			atuador_put_Ni(100.0);
    			atuador_put_Na(100.0);
    			//atuador_put_Nf(200.0);
    		} else {
    			atuador_put_Ni(100.0);
    			atuador_put_Na(0.0);
    			//atuador_put_Nf(100.0);
    	 ...
    	 if(erro>0.2  && temp<ref_temp) { //aumenta temperatura
    ...
    if(erro<0.2 && temp>ref_temp) { //diminuir temperatura
    		atuador_put_Q(0.0);
    		if(Ti>temp && temp>80.0){
    			atuador_put_Ni(0.0);
    			atuador_put_Na(20.0);
...
void thread_controle_nivel(void){
	...
	double niv=0.0, ref_niv=0.0;
	double erro=0.0,no=0.0;
	...
	while(1){
		...
		//if(erro<0.0000) erro = -erro;
		//erro = abs((temp-ref_temp));
		if(niv>ref_niv){ // diminuir volume			
			atuador_put_Ni(0.0);
			atuador_put_Na(0.0);
			atuador_put_Nf(1000000.0);
		}
	        if(niv<ref_niv){ // aumenta volume
	        if(no>0.0){
		  // atuador_put_Ni(no);
		   atuador_put_Na(100.0+no);
		   atuador_put_Nf(0.0);
		} else {
			//atuador_put_Ni(100.0);
			atuador_put_Na(1000000.0);
			atuador_put_Nf(0.0);
			}
		}
